# Contract-First IDP Tenant

`customer-experience` is the tenant Domain owned by
...
The Domain is attached to `resource:default/workshop`. That platform target supplies trusted
...
`dev` is the build environment and must remain first.
